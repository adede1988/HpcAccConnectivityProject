...
pairDat.frex = frex; 
pairDat.stds = stds; 
pairDat.chan1Reg = initDat.chan1Reg{1}; 
pairDat.chan2Reg = initDat.chan2Reg{1};

%get the HFB latencies and reactivity from channel 2
if ~isfield(pairDat, 'react2')
...
LL = struct; 
    ...
    for fi = 1:length(frex)
        disp(num2str(fi))
        [LLsubHit(fi,:,:,:), LLsubMiss(fi,:,:,:)] = getLL_trials(squeeze(pow(:,:,fi)),...
            squeeze(pow2(:,:,fi)), missidx, hitidx, ...
            chanDat.enctim, leadLagEncTim); 
    end
    LL.subMiss_low = squeeze(median(LLsubMiss(1:4, :,1:5:301, :), 1)); 
    LL.subMiss_high = squeeze(median(LLsubMiss(5:8, :,1:5:301, :), 1)); 
    LL.subHit_low = squeeze(median(LLsubHit(1:4, :,1:5:301, :), 1)); 
    LL.subHit_high = squeeze(median(LLsubHit(5:8, :,1:5:301, :), 1)); 
    ...
    LL.retMiss_low = squeeze(median(LLretMiss(1:4, :,1:5:301, :), 1)); 
    LL.retMiss_high = squeeze(median(LLretMiss(5:8, :,1:5:301, :), 1)); 
    LL.retHit_low = squeeze(median(LLretHit(1:4, :,1:5:301, :), 1)); 
    LL.retHit_high = squeeze(median(LLretHit(5:8, :,1:5:301, :), 1)); 
