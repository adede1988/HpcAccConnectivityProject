function [errorChans, outDat] = getAllChanDat2(chanFiles, sub, targVariable, noneFlag)
...
%     sub
    ...
    if isfile(['R:\MSS\Johnson_Lab\dtf8829\SUMDAT\' ...
            chanDat.site '_' chanDat.subID '_' targVariable '.mat'])
...
%         chanDat.allBrod = 1; 
        if isfield(chanDat, 'HFB') && ...
           isfield(chanDat, 'TF2') && ...
           isfield(chanDat, 'reactiveRes')%check for complete processing
            %only reactive channels have leadlag calculations 
             ...
             if sum(chanDat.reactiveRes==1)>0
                curSubReact(chan) = 1; 
%                 curIdx = [curIdx Ei]; 
                if Ei == 1
                    allDat = chanDat; 
