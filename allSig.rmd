...
demoDF <- df[!rowSums(is.na(df[,11:448])) == 438,1:10]
demoDF$reg = as.factor(demoDF$reg)
contrasts(demoDF$reg) <- contr.sum
          
age.lm = lm(age ~ reg, data = demoDF)
summary(age.lm)

sex.glm = glm(as.factor(sex) ~ reg, family = binomial,  data = demoDF)
summary(sex.glm)

memAcc.lm = lm(memAcc ~ reg, data = demoDF)
summary(memAcc.lm)

memD.lm = lm(memD ~ reg, data = demoDF)
summary(memD.lm)
...
demoDF %>% group_by(reg) %>% summarize(N = n(), 
                                   Fem = sum(sex == "F"),
                                   agemean = mean(age), 
                                   agesd = sd(age),
                                   ...
                                   retMisssd = sd(retMISS) ) -> demoDFout

write.csv(demoDFout, "G:\\My Drive\\GitHub\\HpcAccConnectivityProject\\demoTable.csv")

```

## do correlations and plot some histograms 
```{r correlate and hists}
...
```
## correlation histograms broken down by timeSet
```{r timeSet}

#plot histogram by timeSet
corTest %>% filter(type != 'HFB') %>% ggplot(aes(x = corVal, fill = type, color = type)) + 
  geom_histogram(breaks = seq(-1,1,.2)) + facet_grid(.~timeSet) + 
...
for(rr in 1:2){
  ...
  tmpDat <- corTest %>% filter(timeSet == timeSetVals[rr], type != 'HFB')
    
  outPlot <- ggplot(tmpDat, aes(x=corVal, fill = type, color = type)) + 
                  geom_histogram(breaks = seq(-1,1,.25)) +
                  ylab("effect Count") +
                  ylim(c(0,25)) + 
  ...
  png(paste( "R:\\MSS\\Johnson_Lab\\dtf8829\\publicationFigureData\\pubFigs\\" ,
                'fig5','_corHist_',timeSetVals[rr], '.png', 
                sep = ''),         # File name
...
#dif in large v. small effects 
timeSetSum <- corTest %>% filter(type != 'HFB') %>% group_by(timeSet) %>% summarize(above = sum(abs(corVal)>.2), 
                                         below = sum(abs(corVal)<=.2)) 

chisq.test(timeSetSum[,2:3])


```

## correlation histograms broken down by region
```{r region}

#plot histogram by timeSet
corTest %>% filter(type != 'HFB') %>% ggplot(aes(x = corVal, fill = type, color = type)) + 
  geom_histogram(breaks = seq(-1,1,.2)) + facet_grid(reg1~.) + 
  geom_vline(xintercept = 0)
...
for(rr in 1:5){
  ...
  tmpDat <- corTest %>% filter(reg1 == reg_order[rr], type != 'HFB')
  ...
  png(paste( "R:\\MSS\\Johnson_Lab\\dtf8829\\publicationFigureData\\pubFigs\\" ,
                'fig5','_corHist_',reg_color[rr], '.png', 
                sep = ''),         # File name
      width=600, height=600)
  print(outPlot)
  dev.off()
...
#dif in large v. small effects 
timeSetSum <- corTest %>% filter(type != 'HFB') %>% group_by(reg1) %>% summarize(above = sum(abs(corVal)>.2), 
                                         below = sum(abs(corVal)<=.2)) 
...
#dif in pos v. neg effects 
timeSetSum <- test %>% filter(type != 'HFB') %>% group_by(timeSet) %>% summarize(above = sum(corVal>0), 
                                         below = sum(corVal<=0)) 

chisq.test(timeSetSum[,2:3])

#plot histogram by region
test %>% filter(type != 'HFB') %>% ggplot(aes(x = corVal, fill = type, color = type)) + 
  geom_histogram(breaks = seq(-1,1,.2)) + facet_grid(reg1~.) + 
  geom_vline(xintercept = 0)

#dif in large v. small effects
timeSetSum <- test %>% filter(type != 'HFB') %>% group_by(reg1) %>% summarize(above = sum(abs(corVal)>.2), 
                                         below = sum(abs(corVal)<=.2)) 

chisq.test(timeSetSum[,2:3])

#dif in pos v. neg effects
timeSetSum <- test %>% filter(type != 'HFB') %>% group_by(reg1) %>% summarize(above = sum(corVal>0), 
                                         below = sum(corVal<=0)) 
...
corTest %>% filter(type != 'HFB') %>% ggplot(aes(x = corVal, fill = type, color = type)) + 
  geom_histogram(breaks = seq(-1,1,.2)) + facet_grid(type~.) + 
  geom_vline(xintercept = 0)

#dif in large v. small effects
timeSetSum <- test %>% filter(type != 'HFB') %>% group_by(type) %>% summarize(above = sum(abs(corVal)>.2), 
                                         below = sum(abs(corVal)<=.2)) 
...
test %>% filter(type != 'HFB') %>% group_by(timeSet) %>% summarize(n = n(), 
                                                                   meanCor = mean(corVal))



longDF %>% filter(reg == "dlPFC", type == "ppc", regionphase=="_pos_ret_image_dlPFC_mtl_1") %>% ggplot(aes(x = stat, y = memD)) + geom_point()
