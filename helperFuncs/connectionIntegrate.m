function [] = connectionIntegrate(outStatFiles, regii, regjj)



regions = {'acc', 'dlPFC', 'hip', 'lTemp', 'iTemp', 'mtl', 'pcc',...
          'pPFC', 'vis'}; 
phases = {'enc', 'ret'}; 
stat = [0,1]; %0 = HFB locked, 1 = image locked
statName = {'HFB', 'image'};
%down select files to the current target region and phase of experiment

set(0, 'defaultfigurewindowstyle', 'normal')
figure('visible', false, 'position', [100, 100, 800, 2000])
outDat = struct; 
outDat.reg1 = regions{regii}; 
outDat.reg2 = regions{regjj}; 
for pp = 1:2
    for ss = 1:2
        
        curFiles = outStatFiles; 
        if ss == 1
            curFiles(~ismember({curFiles.reg1}, regions{regii}),:) = []; 
            curFiles(~ismember({curFiles.reg2}, regions{regjj}),:) = []; 
        else
            %flip it for image locked data because not all were calcualted
            if regii>regjj
                tmpjj = regii; 
                tmpii = regjj; 
            else
            ...
            curFiles(~ismember({curFiles.reg1}, regions{tmpii}),:) = []; 
            curFiles(~ismember({curFiles.reg2}, regions{tmpjj}),:) = [];
        end
        curFiles(~ismember({curFiles.phase}, phases{pp}),:) = [];
        curFiles(cellfun(@(x) str2num(x), {curFiles.stat}) ... 
            ~= stat(ss), :) = []; 
        ...
        dat = load([curFiles(1).folder '/' curFiles(1).name]).outDat;
        %aggregate the perms
        nullTs = zeros([size(dat.nulls, [1,2]), length(curFiles)*50]);
        for ii = 1:length(curFiles)
            perm = load([curFiles(ii).folder '/' ...
                         curFiles(ii).name]).outDat;
        ...
        outDat.([phases{pp} '_' statName{ss} '_' 'p']) = p; 
        outDat.([phases{pp} '_' statName{ss} '_' 'hitVals']) = dat.hitVals;
        outDat.([phases{pp} '_' statName{ss} '_' 'missVals']) = dat.missVals; 
        outDat.([phases{pp} '_' statName{ss} '_' 'clust']) = clusterinfo;
        outDat.([phases{pp} '_' statName{ss} '_' 'tim']) = dat.tim; 
        outDat.([phases{pp} '_' statName{ss} '_' 'tVal']) = dat.tVals; 
        outDat.frex = dat.frex; 
        %these values are getting overwritten! BUT THEY'RE NOT ALWAYS THE
        %SAME!!!!!!!!! 
        outDat.pairIDs = dat.pairIDs; 
        outDat.subVals = dat.subVals;
        outDat.chiVals = dat.chiVals; 

        subplot(6,2,(pp-1)*6+ss)
        inMat = squeeze(mean(dat.hitVals,3)); 
        inMat(inMat<0) = 0; 
        inMat = sqrt(inMat); 
        makeConnectivityHeatMap(inMat, ones(size(inMat)), ...
                                ss, dat, regions, ...
                                regii, regjj, phases, pp, statName)
        title(['connectivity ' regions{regii} ' ' ...
                regions{regjj} ' hits ' phases{pp}])
%         caxis([.01, .1])

        subplot(6,2,(pp-1)*6+ss + 2)
        inMat = squeeze(mean(dat.missVals,3)); 
        inMat(inMat<0) = 0; 
        inMat = sqrt(inMat); 
        makeConnectivityHeatMap(inMat, ones(size(inMat)), ...
                                ss, dat, regions, ...
...
%         caxis([.01, .1])
        ...
        subplot(6,2,(pp-1)*6+ss + 4)
        inMat = squeeze(dat.tVals); 
        makeConnectivityHeatMap(inMat, p, ...
                                ss, dat, regions, ...
                                regii, regjj, phases, pp, statName)
        title([' t-values '])
        caxis([1, 3])
...
path='G:\My Drive\Johnson\MTL_PFC_networkFigs\Connectivity_inferentials\'; 

export_fig([path regions{regii} '_' regions{regjj} '.jpg'], ...
    '-r300')

path = 'R:\MSS\Johnson_Lab\dtf8829\publicationFigureData\Figure3\';
save([path regions{regii} '_' regions{regjj} '_connectionDat.mat'],...
    'outDat')
...
%         title(['connectivity ' regions{regii} ' ' ...
%             regions{regjj} ' hit - miss ' phases{pp}])
%         ylabel('frequency')
