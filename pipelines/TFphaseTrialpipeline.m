...
%                 'startHitVals' '.mat'], 'hitVals')
        ...
        for chan = 1:length(chanUni)
            ...
            phaseVals = hitTrials(:, hitidx, :); 
            [tL, trialL, frexL] = size(phaseVals);
            %compute ITPC
            test = arrayfun(@(y) arrayfun(@(x) trialL*...
                abs(mean(exp(1i*squeeze(phaseVals(x,:,y)) ))).^2,...
                1:tL ), 1:frexL, 'uniformoutput', false);
            test = cat(1, test{:}); 
            hitVals(chan, :,:) = test'; 
            
            phaseVals = missTrials(:, missidx, :); 
            [tL, trialL, frexL] = size(phaseVals);
            %compute ITPC
            test = arrayfun(@(y) arrayfun(@(x) trialL*...
                abs(mean(exp(1i*squeeze(phaseVals(x,:,y)) ))).^2,...
...
%        HFBdat.p_image = p; 
       ...
       save([statFiles(fileIdx).folder '/out/NEWphase'...
...
%         tim(tim<-450 | tim>3000) = []; 
        ...
        for chan = 1:length(chanUni)
            ...
            phaseVals = hitTrials(:, :, hitidx); 
            phaseVals = permute(phaseVals, [1,3,2]); 
            [tL, trialL, frexL] = size(phaseVals);
            %compute ITPC
            test = arrayfun(@(y) arrayfun(@(x) trialL*...
                abs(mean(exp(1i*squeeze(phaseVals(x,:,y)) ))).^2,...
